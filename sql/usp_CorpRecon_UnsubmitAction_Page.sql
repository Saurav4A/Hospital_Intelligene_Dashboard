/* Corporate reconciliation unsubmit action desk paging SP */
IF OBJECT_ID(N'dbo.usp_CorpRecon_UnsubmitAction_Page', N'P') IS NULL
    EXEC(N'CREATE PROCEDURE dbo.usp_CorpRecon_UnsubmitAction_Page AS BEGIN SET NOCOUNT ON; END;');
GO
ALTER PROCEDURE dbo.usp_CorpRecon_UnsubmitAction_Page
    @CutoffDate DATE = '2025-03-31',
    @Q NVARCHAR(200) = N'',
    @BillSource NVARCHAR(120) = N'Corporate Bill',
    @Page INT = 1,
    @PageSize INT = 25
AS
BEGIN
    ...
    DECLARE @BillSourceSafe NVARCHAR(120) = LOWER(LTRIM(RTRIM(ISNULL(@BillSource, N'Corporate Bill'))));
    ...
    SELECT
        ...
        ISNULL(a.writeoff_total_all_time, 0) AS writeoff_total_all_time,
    ...
    INTO #rows_filtered
    FROM #rows_all r
    WHERE LOWER(ISNULL(r.BillSource, N'')) = @BillSourceSafe
      AND UPPER(ISNULL(r.BillStatusRaw, N'')) = N'Y'
      AND (
            @QSafe = N''
            OR (
                @HasTermList = 1
    ...
    SELECT N'Corporate Bill' AS bill_source;
END;
GO
